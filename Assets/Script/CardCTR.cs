...
public class CardCTR : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Start()
    {
        ...
        manager = battleManager.GetComponent<BattleManager>();
    }
    ...
    public void OnDrag(PointerEventData eventData){
        ...
        // 使用可能ならターゲットを取得
        if(manager.mana >= cardData.cardCost && rectTransform.anchoredPosition.y >= 180){
    ...
    public void OnEndDrag(PointerEventData eventData){
        ...
        if(manager.mana >= cardData.cardCost && !pos_reset){
            // カードを使用する処理をここに追加
            ...
            foreach (var effect in cardData.effectList){
                ...
                if(!pos_reset) manager.CardUsing(this.gameObject, target);
            }
        }
        else pos_reset = true;
        */
        ...
        if(!pos_reset) {
            manager.CardUsing(this.gameObject, target);
        }
        // そうでないなら、カードを元の位置に戻す
        else{
